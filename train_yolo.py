...
def parse_args():
    ...
    parser.add_argument('--cpus', type=str, default='0',
                        help='Training with CPUs, you can specify 1,3 for example.')
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = VOCLike(root='D:\BaiduNetdiskDownload', splits=[(2028, 'trainval')])
        val_dataset = VOCLike(root='D:\BaiduNetdiskDownload', splits=[(2028, 'test')])
...
if __name__ == '__main__':
    ...
    # training contexts
    ctx = [mx.cpu(int(i)) for i in args.cpus.split(',') if i.strip()]
